...
import org.tygus.suslik.certification.targets.coq.language.Statements.{CIf, CSeqComp, CStatement}
...
object StatementProducers {
  ...
  case class PartiallyAppliedCStmtProducer(p: CStmtProducer, s: CStatement) extends CStmtProducer {
  ...
  case class BranchCStmtProducer(selectors: Seq[CExpr]) extends CStmtProducer {
  ...
  case class FoldCStmtProducer[T](op: (T, CStmtProducer) => CStatement, item: T, bp: CStmtProducer) extends CStmtProducer {
    ...
    val fn: Kont = steps => {
      ...
      def isBase(curr: CStmtProducer): Boolean = curr match {
        ...
        case _: BranchCStmtProducer => true
        case _ => false
      }
      def update(curr: CStmtProducer): CStmtProducer = curr match {
        ...
        case _: PartiallyAppliedCStmtProducer | _: BranchCStmtProducer if isBase(curr) => curr.partApply(steps.head)
        case _ => curr
      }
      op(item, update(bp))
    }
