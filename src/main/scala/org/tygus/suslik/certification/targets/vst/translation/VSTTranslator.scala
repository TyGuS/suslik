...
import org.tygus.suslik.certification.targets.vst.clang.Expressions
import org.tygus.suslik.certification.targets.vst.logic.ProofTerms.{IsValidPointerOrNull, VSTPredicate}
import org.tygus.suslik.certification.targets.vst.logic.VSTProofStep.{Forward, ForwardIf, ValidPointer}
import org.tygus.suslik.certification.targets.vst.logic.{ProofTerms, ProofTypes, VSTProofStep}
...
import org.tygus.suslik.language.{Expressions, SSLType}

import scala.collection.immutable.Queue

case class VSTClientContext(
                             pred_map: Map[String, VSTPredicate],
                             coq_context: Map[String, ProofTypes.VSTProofType],
                             variable_map: Map[String, ProofTerms.Expressions.ProofCExpr]
                           ) extends ClientContext[VSTProofStep] {
  def with_variables_of(variables: List[(String, SSLType)]): VSTClientContext = {
    val new_vars: Map[String, ProofTypes.VSTProofType] = variables.map({ case (name, ty) => (name, ProofSpecTranslation.translate_type(ty)) }).toMap
